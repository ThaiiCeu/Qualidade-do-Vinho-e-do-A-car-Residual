...
# Precisa converter para factor a variável y

class(train$quality_class)
train$quality_class = as.factor(train$quality_class)

rf_model <- randomForest(quality_class ~ ., data = train[, c(1:11, 12)], ntree = 100)
pred_rf <- predict(rf_model, test[, 1:11])

# Calcular a matriz de confusão
conf_matrix_rf <- table(Predicted = pred_rf, Actual = test$quality_class)

# Extração dos valores da matriz de confusão focada na classe "bom" como positivo
TP <- conf_matrix_rf["bom", "bom"]  # True Positives (Predito como bom e é bom)
FP <- conf_matrix_rf["bom", "ruim"]  # False Positives (Predito como bom, mas é ruim)
FN <- conf_matrix_rf["ruim", "bom"]  # False Negatives (Predito como ruim, mas é bom)
TN <- conf_matrix_rf["ruim", "ruim"]  # True Negatives (Predito como ruim e é ruim)

# Total de observações
total <- sum(conf_matrix_rf)

# Cálculo das métricas
accuracy <- (TP + TN) / total  # Acurácia
precision <- TP / (TP + FP)  # Precisão (Valor Preditivo Positivo)
npv_rf <- TN / (TN + FN)  # Valor Preditivo Negativo
...
pe <- ((sum(conf_matrix_rf[, "bom"]) * sum(conf_matrix_rf["bom", ])) +
         (sum(conf_matrix_rf[, "ruim"]) * sum(conf_matrix_rf["ruim", ]))) / (total^2)  # Esperado
...
# Exibição dos resultados focados no evento de interesse 'bom'
cat("Métricas do Modelo Random Forest (Evento de Interesse: 'bom'):\n")
...
cat("Valor Preditivo Negativo (NPV):", npv_rf, "\n\n")
...
conf_matrix_rf_df <- as.data.frame(as.table(conf_matrix_rf))

# Plotar a matriz de confusão com ggplot2
ggplot(data = conf_matrix_rf_df, aes(x = Actual, y = Predicted, fill = Freq)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "red") +
  geom_text(aes(label = Freq), color = "black", size = 5) +
  labs(
    title = "Matriz de Confusão - Modelo Random Forest",
    x = "Classe Real",
    y = "Classe Prevista",
    fill = "Frequência"
  ) +
...
#reg
rf_reg <- randomForest(train_labels ~ ., data = train_data[, 1:11], ntree = 100)
pred_rf_reg <- predict(rf_reg, test_data[, 1:11])

r2 <- 1 - (sum((test_labels - pred_rf_reg)^2) / sum((test_labels - mean(test_labels))^2))

mse = mean((test_labels - pred_rf_reg)^2) 

rmse <- sqrt(mse)

mae <- mean(abs(test_labels - pred_rf_reg))

mape <- mean(abs((test_labels - pred_rf_reg) / test_labels)) * 100
